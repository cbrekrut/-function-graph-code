...
#define PARSER_H
////////////////////////////////////////////////////////////
/// Я понимаю что добавление .h файла в проект влияет на скорость компияции и размера файла
/// не в лучшую сторону, но это было необходимо для того чтоб показать хороший clean-code
/////////////////////////////////////////////////////////////
...
bool Maths(stack <Leksema>& Stack_n, stack <Leksema>& Stack_o, Leksema& item) { //Математическая функция, которая производит расчеты
	 QMessageBox msgBox;
	//Функция имеет тип bool, чтобы при возникновении какой-либо ошибки возвращать "false" и выводить qmessagebox
	double a, b, c;
	...
	case '/':
		b = Stack_n.top().value;
		if (a == 0) {
			msgBox.setText("На 0 делить нельзя!");
			msgBox.exec();
			return false;
		}
		else {
	...
	case 't':
		if (Cos(a) == 0) {
		    msgBox.setText("Невалидное выражение!");
		    msgBox.exec();
			return false;
		}
		else {
...
float parser(string str,double xmin)
{
  ...
  while (true) {
      ...
      if (Ch == 'x') { //Если прочитана неизвестная
          item.type = '0';
          item.value = xmin;
          Stack_n.push(item); //Число кладется в стек с числами
          flag = 0;
  ...
  while (Stack_o.size() !=
      ...
      if (Maths(Stack_n, Stack_o, item) == false) { //Если функция вернет "false", то прекращаем рабоу
          return 0;
      } else continue; //Если все хорошо
  }
